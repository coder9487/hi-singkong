...
export class FirstPersonCameraControl_ForSwordfish {
  ...
  mobileMove() {
    let MobileMovement = store.state.MobileMovement;
    let MobileDirection = store.state.MobileDirection;
    // switch (moveForward) {
    //   case false: // w
    //     this._camerLocalDirection.z = 0;
    //     break;
    ...
    if (MobileMovement.y == 0) this._camerLocalDirection.z = 0;
    else if (MobileMovement.y < -3) this._camerLocalDirection.z = 1;
    else if (MobileMovement.y > 3) this._camerLocalDirection.z = -1;
    if (MobileMovement.y == 0) this._camerLocalDirection.x = 0;
    else if (MobileMovement.x > 3) this._camerLocalDirection.x = 1;
    else if (MobileMovement.x < -3) this._camerLocalDirection.x = -1;


    this.rotateX(MobileDirection.hori);
    this.rotateY(MobileDirection.vert);
        ...
        if (this.positionEasing) {
          if (
            ...
            this.camera.position.y +=
              (newPosition.y - this.camera.position.y) * 0.08; //調上升大小
            this._fallingTime = 0;
            isFalling = false;
            return;
          }
